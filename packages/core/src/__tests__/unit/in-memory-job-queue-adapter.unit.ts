...
import {Job} from '../../job';
import {JobListOptions, SortOrder} from '../../types';

describe('InMemoryJobQueueAdapter', () => {
  let strategy: InMemoryJobQueueAdapter;
  beforeEach(async () => {
    strategy = new InMemoryJobQueueAdapter({
      processMode: 'worker',
    });
    await strategy.init();
  });
  ...
  describe('findMany options', () => {
    ...
    it('filter queueName', async () => {
      expect(await getIdResultsFor({filter: {queueName: {eq: 'video'}}})).toEqual(['video-1', 'video-2', 'video-3']);
